    ...
    if (req.method === "GET") {
      try {
        const result = await pool.query(
          `SELECT id, subject, sender, email_date, summary, created_at 
  ...
  "conclusion": "phrase de conclusion en 2-3 phrases max"
        ...
        await pool.query(
          `INSERT INTO market_summaries (subject, sender, email_date, summary)
           VALUES ($1, $2, $3, $4)`,
          [subject, sender, date, JSON.stringify(summary)]
        );

        return res.status(200).json({ ok: true, summary });
      } catch (e: any) {
        console.error("market_summary error:", e);
        return res.status(500).json({ error: e.message });
      }
    }
  ...
  if (section === "samdp_debt") {
    if (req.method === "GET") {
      try {
        ...
        const importId = logRes.rows[0].id;
const instruments = await pool.query(
          `SELECT * FROM samdp_debt_instruments WHERE import_id=$1 ORDER BY id ASC`,
          [importId]
        );
        return res.json({ instruments: instruments.rows, importLog: logRes.rows[0] });
      } catch (e: any) {
